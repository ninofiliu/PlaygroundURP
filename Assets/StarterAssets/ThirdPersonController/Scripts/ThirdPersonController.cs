...
public class ThirdPersonController : MonoBehaviour {
  ...
  private void Move() {
    ...
    // a simplistic acceleration and deceleration designed to be easy to remove,
    // replace, or iterate upon

    // note: Vector2's == operator uses approximation so is not floating point
    // error prone, and is cheaper than magnitude if there is no input, set the
    // target speed to 0
    ...
    if (currentHorizontalSpeed < targetSpeed - speedOffset ||
      ...
      // creates curved result rather than a linear one giving a more organic
      // speed change note T in Lerp is clamped, so we don't need to clamp our
      // speed
    ...
    // note: Vector2's != operator uses approximation so is not floating point
    // error prone, and is cheaper than magnitude if there is a move input
  ...
  private void JumpAndGravity() {
    if (Grounded) {
      ...
      if (_input.jump && _jumpTimeoutDelta <= 0.0f) {
        // the square root of H * -2 * G = how much velocity needed to reach
        // desired height
    ...
    // apply gravity over time if under terminal (multiply by delta time twice
    // to linearly speed up over time)
  ...
  private void OnDrawGizmosSelected() {
    ...
    // when selected, draw a gizmo in the position of, and matching radius of,
    // the grounded collider
  ...
  private void OnFootstep(AnimationEvent animationEvent) {
    if (animationEvent.animatorClipInfo.weight > 0.5f) {
      if (FootstepAudioClips.Length > 0 && _controller != null) {
